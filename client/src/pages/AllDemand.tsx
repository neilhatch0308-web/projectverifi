        ...
        <table className="data-table">
          ...
          <tbody>
            {filtered.map((d) => (
              <tr key={d.id}>
                <td>
                  <Link to={`/demand/${d.id}`} style={{ color: 'inherit', textDecoration: 'none' }}>
                    {d.title}
                  </Link>
                </td>
                <td>
                  <span className={`pill ${statusPill[d.status] ?? 'pill--muted'}`}>{d.status}</span>
                </td>
                <td>{d.total_score > 0 ? d.total_score : <span style={{ color: 'var(--muted)' }}>-</span>}</td>
